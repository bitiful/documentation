...
1. **超高性能**：性能最多可达国内主流图片处理性能的 **10 倍**；
2. **独家支持颜色管理**：国内唯一可准确处理 **CMYK 色彩模式** 的实时媒体引擎；
3. **独家超大图处理**：CoreIX 允许高性能处理 **高达 200MB** 的图片；
4. **独家支持 HDR 视频截图**：可以输出 **HDR 图片**，也可映射为 **SDR 图片**；
5. **独家画面感知能力**：在必须裁剪画面时可 **自动保留画面主体**；
6. **独家原生支持 SVG**：可以将 SVG 转成光栅图，也可作为 **水印图**；
7. **独家哈希占位支持**：原生支持 **Thumbhash** 和 **Blurhash** 码输出；
8. **独家原生支持 RAW 格式**：兼容主流 RAW 格式，并自动进行恰当的 **Post-processing**；
...
| [格式转换](/coreix/features/fmt)          | - fmt: 格式代号            | 图片格式：jpeg、png、tpng、webp、bmp、avif、gif、tiff  - 其他格式：blurhash、thumbhash、json（输出图片、视频信息）                   | 全部     |
